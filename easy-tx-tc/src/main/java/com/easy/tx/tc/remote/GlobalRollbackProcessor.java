 ...
 * @date 2023/08/22
 */
public class GlobalRollbackProcessor implements RemoteMessageProcessor {
    
    private final TcGlobalExecutor globalConmmitManager;
    
    public GlobalRollbackProcessor(TcGlobalExecutor globalConmmitManager) {
    ...
    public void process(RemoteResponse response, RemoteMessage message) {
        ...
        globalConmmitManager.rollback(globalTxSession);
        
        //返回结果
        RemoteMessage responseMessage = RpcMessageUtil.buildResponseMessage(null, getType());
        response.setRpcMessage(responseMessage);
    }
    
    @Override
    public Integer getType() {
        return MessageType.GLOBAL_ROLLBACK;
    }
}
